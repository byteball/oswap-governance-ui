...
class httpHub {
	constructor() {
		this.hubUrl = `https://${process.env.REACT_APP_ENVIRONMENT !== "livenet" ? "testnet." : ""}obyte.org/api`;
	...
	async getStateVars(address, var_prefix, var_prefix_from) {
		return await this.client.post("/get_aa_state_vars", { address, var_prefix, var_prefix_from }).then((res) => res?.data?.data || {});
	}

    async executeGetter(address, getter, params) {
        return await this.client.post("/execute_getter", { address, getter, params }).then((res) => res?.data?.data || {});
    }
