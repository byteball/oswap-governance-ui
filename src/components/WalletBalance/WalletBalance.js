...
export const WalletBalance = () => {
  ...
  return <div className={styles.walletWrap}>
    {walletAddress ? <div>
      ...
      <div>Locked balance: {+Number(balance / 10 ** decimals).toFixed(decimals)} {symbol === poolAsset.replace(/[+=]/, '').substr(0, 6) ? <a href={`${process.env.REACT_APP_EXPLORER_LINK}${poolAsset}`} target="_blank" rel="noopener">{symbol}</a> : symbol} </div>
