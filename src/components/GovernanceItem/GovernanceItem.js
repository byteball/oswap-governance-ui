...
const viewParam = ({ value, name, mid_price_decimals, max_decimals }) => {
  if (name === "alpha") {
    const inPercent = +Number(value * 100).toFixed(4);
    return `${inPercent}% / ${100 - inPercent}%`;
  } if (name === "mid_price" && mid_price_decimals !== undefined){
    return +Number(value / 10 ** mid_price_decimals).toFixed(max_decimals);
...
export const GovernanceItem = (props) => {
  const { name, value, activeGovernance, choice, leader, voteTokenDecimals, voteTokenSymbol, challenging_period, freeze_period, supports = {}, challenging_period_start_ts, change, balance, activeWallet, mid_price_decimals, max_decimals } = props;

  const description = paramList[name]?.description || "No description";
  const rule = paramList[name]?.rule || "No rule";

  const valueView = viewParam({ name, value, mid_price_decimals, max_decimals });
  const choiceView = viewParam({ name, value: choice, mid_price_decimals, max_decimals });
  const leaderView = viewParam({ name, value: leader, mid_price_decimals, max_decimals });
